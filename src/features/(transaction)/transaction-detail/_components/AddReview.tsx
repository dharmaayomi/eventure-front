...
const AddReview: FC<AddReviewProps> = ({ uuid }) => {
  ...
  return (
    <Dialog.Root>
      <Dialog.Trigger className="rounded-lg bg-indigo-600 px-4 py-2 text-white transition hover:bg-indigo-700">
        Add Review
      </Dialog.Trigger>

      <Dialog.Portal>
        <Dialog.Overlay className="fixed inset-0 bg-black opacity-50" />
        <Dialog.Content className="fixed top-1/2 left-1/2 w-full max-w-md -translate-x-1/2 -translate-y-1/2 transform rounded-lg bg-white p-6 shadow-lg">
          <Dialog.Title className="mb-4 text-xl font-bold text-indigo-700">
            Add Review
          </Dialog.Title>
